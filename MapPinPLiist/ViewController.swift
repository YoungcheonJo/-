...
class ViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let path = Bundle.main.path(forResource: "data1", ofType: "plist")
        ...
        if let myItems = contents {
            ...
            for item in myItems {
                ...
                let title = (item as AnyObject).value(forKey: "parkName")
                let subTitle = (item as AnyObject).value(forKey: "addrRoad")
                //let img = (item as AnyObject).value(forKey: "img")
                
                let annotation = MKPointAnnotation()
                
                print("lat = \(String(describing: lat))")
                if lat == nil || long == nil || subTitle == nil {
    ...
    func zoomToRegion() {
        ...
        case 0:
            ...
            myMapView.setRegion(region, animated: true)
        case 1:
           span = MKCoordinateSpanMake(0.052, 0.052)
           let region = MKCoordinateRegionMake(center, span)
